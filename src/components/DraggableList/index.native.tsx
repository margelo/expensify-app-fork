...
function DraggableList<T>({ref, data, renderItem, keyExtractor, onDragEnd, isItemDragDisabled, ListFooterComponent, disableScroll = false}: DraggableListProps<T> & {ref?: React.Ref<DraggableListRef>}) {
    ...
    const updateDragPosition = (index: number, gestureTranslationY: number) => {
        ...
        const listState = listRef.current?.getState();
        const viewportSize = viewportSizeRef.current || listState?.scrollLength || 0;
        const contentSize = contentSizeRef.current || listState?.contentLength || 0;
