...
function handleOnyxUpdateGap(onyxUpdatesFromServer: OnyxEntry<OnyxUpdatesFromServer>, clientLastUpdateID?: number) {
    ...
    if (Object.values(deferredUpdatesProxy.deferredUpdates).length === 0) {
    ...
    if (!lastUpdateIDFromClient) {
        ...
        console.log('update', {updates: deferredUpdatesProxy.deferredUpdates, length: Object.keys(deferredUpdatesProxy.deferredUpdates).length});
        ...
        console.log(`INITIAL: GetMissingOnyxMessages from ${lastUpdateIDFromClient} to ${previousUpdateIDFromServer}`);
        ...
        queryPromise = new Promise((resolve, reject) => {
            setTimeout(() => {
                ...
                App.getMissingOnyxUpdates(lastUpdateIDFromClient, previousUpdateIDFromServer)
                    .then(() => {})
                    .then(resolve)
                    .catch(reject);
            }, 20000);
        }).then(() => OnyxUpdateManagerUtils.validateAndApplyDeferredUpdates(clientLastUpdateID));
    }
